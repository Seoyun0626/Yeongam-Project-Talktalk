...
import { IChangePassword, User} from '../interfaces/user_interface';
...
export const createUser = async ( req: Request, res: Response ): Promise<Response> => {
    ...
    try {
        ...
        //username - user_name, fullname : user_id
        const { user_name, user_id, user_email, user_pw }: User = req.body;

        const conn = await connect();

        const [existsEmail] = await conn.query<RowDataPacket[]>('SELECT email FROM users WHERE email = ?', [ user_email ]);
        ...
        await sendEmailVerify('확인 코드', user_email, `<h1> Yeongam Mobile </h1><hr> <b>${ randomNumber } </b>`);
