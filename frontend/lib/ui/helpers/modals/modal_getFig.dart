...
Future<List<EventData>> _getEventData(String eid) async {
  ResponseEvent EventData = await codeService.getEventData(eid);

  return EventData.eventData;
}

void modalGetFig(BuildContext context, String eid) {
  _getEventData(eid).then((List<EventData> value) {
    String message = '';
    String figPayment = '';

    if (value.isNotEmpty) {
      figPayment = value[0].fig_payment.toString();
    }
    ...
    // codeService.getEventData(eid).then((response) {
    ...
    //         message = '';
    //     }
    //     figPayment = events[0]['fig_payment'].toString();
